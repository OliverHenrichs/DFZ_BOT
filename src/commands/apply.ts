...
import {getGuildFromMessage, reactNegative, reactPositive,} from "../misc/messageHelper";
...
export default async (client: DFZDiscordClient, message: Message) => {
    const reactionData: IReactionData = {
        reply: "You just signed up :) Thanks!",
        deleteMessage: false,
        message
    }
    try {
...
async function tryApplyUser(message: Message, client: DFZDiscordClient) {
    ...
    const player = await serializer.get();
    ...
    if (refTag) await handleReferrerTag(guildClient, refTag);
...
function getReferralTag(message: string): string | undefined {
    const args = getTrimmedMessageArguments(message);
    const referralTag = getRefTagFromArgs(args);
