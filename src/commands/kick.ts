...
import {findLobbyByMessage, getArguments, reactPositive,} from "../misc/messageHelper";
...
export default async (message: Message, dbClient: DFZDataBaseClient) => {
    try {
        ...
        await reactPositive({message, reply: "Failed kicking player:" + err, deleteMessage: false});
    }
};
...
async function getKickSpecificsFromMessage(
    ...
    const kickeeIndex = getKickeeIndex(lobby, kickArguments.userId);
...
async function kickPlayerAndReact(
    ...
    await reactPositive({message, reply: "Kicked player.", deleteMessage: true});
}
