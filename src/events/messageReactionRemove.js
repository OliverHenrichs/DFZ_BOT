...
async function handleLobbyRelatedEmoji(client, reaction, user) {
    ...
    if (lri.lobby === undefined ||
        ...
        lri.role === undefined)
        return;
    if (rM.adminRoles.includes(lri.role.id) &&
        reaction.emoji.name === c.lobbyManagementReactionEmojis[2]) {
        lM.removeCoach(client.dbHandle, reaction.message.channel, lri.lobby, user.id)
        ...
        lM.updatePlayerInLobby(client.dbHandle, reaction, lri.lobby, user);
}
/**
