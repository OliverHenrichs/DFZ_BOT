...
import {isSimpleLobbyType, lobbyManagementReactionEmojis, tryoutReactionEmoji,} from "../misc/constants";
...
async function handleMessageReactionRemove(
    ...
    if (ChannelManager.scheduleChannels.includes(reaction.message.channel.id))
        await ScheduleManipulator.removeCoachFromSchedule({client, reaction, user});
