import {GuildMember, MessageReaction, Role, TextBasedChannels, User,} from "discord.js";
import {ChannelManager} from "../logic/discord/DFZChannelManager";
import {DFZDiscordClient} from "../logic/discord/DFZDiscordClient";
import {adminRoles, beginnerRoles, findRole,} from "../logic/discord/roleManagement";
...
async function handleLobbyRelatedEmoji(
    ...
    if (isKnownPositionEmoji(reaction.emoji))
        ...
        changedLobby = await handleSimpleLobbyEmoji(lri.lobby, user, reaction, lri.role);
...
async function addUserOrPosition(
    ...
    const userIdx = getUserIndex(lobby, user.id);
    if (userIdx === -1) {
        addUser(
            lobby,
            user.username,
...
async function handleSimpleLobbyEmoji(
    ...
    if (lobby.type === lobbyTypes.tryout && role.id !== process.env.TRYOUT) {
        await user.send("⛔ You cannot join because you do not have the tryout role.");
        return false;
    }
