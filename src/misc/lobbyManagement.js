...
function getTeamTable(assignedUsers, lobbyType, mention = false) {
    ...
    else if (lobbyType == c.lobbyTypes.unranked ||
...
async function updateAndUnpinLobbyEmbedding(messageId, channel, titleUpdate, unpin = true) {
    ...
    var new_embed = new Discord.MessageEmbed(old_embed).setTitle(newEmbedTitle);
    // update embed
...
function notifyPlayers(client, lobby, playerCount, message) {
    for (let i = 0; i < Math.min(lobby.users.length, playerCount); i++) {
        client.users
            ...
            .catch((err) => console.log("Error notifying players. Errormessage: " + err));
    }
}
function getLobbyPostText(lobbyBeginnerRoles, lobbyType, lobbyRegionRole, coaches) {
    const maxCoachCount = getCoachCountByLobbyType(lobbyType);
    const coachCount = coaches === undefined ? 0 : coaches.length;
    var coachString = "";
    var coachStringPl = "";
...
module.exports = {
     ...
     */
    postLobby: async function (dbHandle, channel, coachIDs, lobbyType, lobbyBeginnerRoles, lobbyRegionRole, zonedTime) {
        var title = "We host " +
        ...
        var text = getLobbyPostText(lobbyBeginnerRoles, lobbyType, lobbyRegionRole, coachIDs);
        ...
        dB.insertLobby(dbHandle, new l.Lobby(lobbyType, zonedTime.epoch, coachIDs, lobbyBeginnerRoles, lobbyRegionRole, channel.id, lobbyPostMessage.id));
    },
    ...
    updateLobbyPost: async function (lobby, channel) {
        ...
        new_embed.description =
            getLobbyPostText(lobby.beginnerRoleIds, lobby.type, lobby.regionId, lobby.coaches) + (timeString !== "" ? "\n" + timeString : "");
    ...
    updateLobbyTimes: async function (guild, dbHandle) {
        ...
        for (const lobby of lobbies) {
            ...
            if (description.length > 0 &&
                description[description.length - 1].startsWith(remainingLobbyTimeStartString))
                description.pop();
            // get new time
            ...
            if (remainingMs > 0) {
                ...
                description.push(remainingLobbyTimeStartString +
                    (hours > 0 ? hours + "h " : "") +
                    minutes +
                    "min");
            }
                ...
                if (hours >= 3) {
                    ...
                    if (description[description.length - 1].startsWith(alreadyStartedLobbyTimeStartString))
                        description.pop();
                    description.push(alreadyStartedLobbyTimeStartString +
                        (hours > 0 ? hours + "h " : "") +
                        minutes +
                        "min ago");
                }
    ...
    startLobby: function (client, lobby, user, channel) {
        ...
        if (timeLeftInMS > fiveMinInMs) {
            // 5min = 300.000 ms
            user.send("It's not time to start the lobby yet (" +
     ...
     * @param {Lobby} lobby
     */
    updateLobbyParameters: function (args, lobby) {
        ...
        while (arguments.length > 0) {
            ...
            if (updateTiers) {
                ...
                const maxTier = 4;
                var res = false, numbers = [], errormsg = "";
    ...
    addCoach: async function (dbHandle, channel, lobby, userId) {
        return new Promise(function (resolve, reject) {
            ...
            updateLobbyPostAndDBEntry(lobby, channel, dbHandle).then(() => resolve(true));
        });
    },
    /**
