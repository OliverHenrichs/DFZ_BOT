...
function getWeekScheduleString(scheduleSetup) {
  return (
    ...
    " in " +
    (scheduleSetup.mondayDate.getYear() + 1900) +
    " (" +
...
async function handleScheduleCoachAdd(client, reaction, user, schedule) {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      await insertScheduledLobbies(
        reaction.message.channel.guild.channels,
        client.dbHandle
      );
...
module.exports = {
  ...
  createLobbySchedules: function (
    ...
    for (let i = 0; i < scheduleSetup.regions.length; i++) {
      ...
      for (let j = 0; j < days.length; j++) {
        ...
        if (_date === undefined) {
          console.log(
            "Could not determine scheduled date for " + scheduleSetup
          );
          return;
        }
  ...
  updateSchedules: async function (dbHandle, channels) {
    ...
    if (dbHandle.dfz_debugMode === true) {
      ...
      if (isNaN(saved_day)) res = await dB.insertDay(dbHandle, day);
      else res = await dB.updateDay(dbHandle, day);
    ...
    [
      monday,
      sunday,
    ] = tZ.getNextMondayAndSundayDate(/*new Date(now.setDate(now.getDate()+21))*/);

    // lobby schedule
    var channel5v5 = channels.cache.find((chan) => {
      return chan.id == cM.scheduleChannel5v5;
    });
    if (channel5v5) {
      var coachCount = 2;
      var days = [[3, 5, 0]];
    ...
    // tryout schedule
    var channelTryout = channels.cache.find((chan) => {
      return chan.id == cM.scheduleChannelTryout;
    });
    if (channelTryout) {
      var coachCount = 1;
      var days = [[2, 4, 6]];
    ...
    // botbash schedule
    var channelBotbash = channels.cache.find((chan) => {
      return chan.id == cM.scheduleChannelBotbash;
    });
    if (channelBotbash) {
      var coachCount = 1;
      var days = [[2, 4, 6]];
  ...
  addCoach: async function (client, reaction, user) {
    ...
    const guildMember = await reaction.message.channel.guild.members.fetch(
      user.id
    );
