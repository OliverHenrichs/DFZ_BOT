...
function reactWithScheduleEmojis(message, lastEmojiIndex = -1) {
    for (let idx = 0; idx <
        (lastEmojiIndex == -1 ? scheduleReactionEmojis.length : lastEmojiIndex); idx++)
...
async function writeSchedule(channel, scheduleSetup) {
    if (scheduleSetup.regions.length !== scheduleSetup.timezones.length ||
    ...
    for (let i = 0; i < scheduleSetup.regions.length; i++) {
        ...
        schedules.push(getScheduleText(dayShortNames[i], emojis, "**" + regionString + " " + scheduleSetup.type + "**", scheduleSetup.times[i], tz, scheduleSetup.coachCount));
...
async function createScheduledLobby(channels, dbHandle, schedule) {
    ...
    var lobbyBeginnerRoles = undefined;
    ...
    schedule.coaches.forEach((c) => {
        ...
        channel.guild.members
            .fetch(c)
            .then((guildMember) => guildMember.send("I just posted tonight's " +
            schedule.region +
            " lobby starting *" +
...
async function handleScheduleCoachAdd(client, reaction, user, schedule) {
    return new Promise(async function (resolve, reject) {
        try {
            ...
            if (guild === null)
                throw "Did not find guild in handleScheduleCoachAdd";
...
function createLobbySchedules(dbHandle, messageId, channelId, scheduleSetup) {
    var dayBaseIndex = 0;
    for (let i = 0; i < scheduleSetup.regions.length; i++) {
        ...
        for (let j = 0; j < days.length; j++) {
            ...
            dB.insertSchedule(dbHandle, new s.Schedule(channelId, messageId, scheduleSetup.type, scheduleSetup.coachCount, reactionEmoji, _date, region));
        }
        dayBaseIndex += days.length;
    }
}
async function createSchedules(dbHandle, channels, chanId, coachCount, days, times, monAndSun) {
    var channel = channels.cache.find((chan) => {
        return chan.id == chanId;
    });
    if (channel !== undefined && channel.isText()) {
        var scheduleSetup = createScheduleSetup(monAndSun.monday, monAndSun.sunday, days, scheduleTypes.lobby, coachCount, times);
        if (scheduleSetup === undefined) {
            console.log("ScheduleSetup creation returned 'undefined' in updateSchedules");
            return;
        }
    ...
    updateSchedules: async function (dbHandle, channels) {
        ...
        if (dbHandle.dfz_debugMode === true) {
            ...
            if (saved_day === day)
                return;
            if (isNaN(saved_day))
                await dB.insertDay(dbHandle, day);
            else
                await dB.updateDay(dbHandle, day);
        ...
        var monAndSun = tZ.getNextMondayAndSundayDate( /*new Date(now.setDate(now.getDate()+21))*/);
        // lobby schedule
        var coachCount5v5 = 2;
        var days5v5 = [[3, 5, 0]];
        var times5v5 = [
        ...
        createSchedules(dbHandle, channels, cM.scheduleChannel5v5, coachCount5v5, days5v5, times5v5, monAndSun);
        // tryout schedule
        var coachCountTryout = 1;
        var daysTryout = [[2, 4, 6]];
        var timesTryout = [["8:00pm", "8:00pm", "8:00pm"]];
        createSchedules(dbHandle, channels, cM.scheduleChannelTryout, coachCountTryout, daysTryout, timesTryout, monAndSun);
        // botbash schedule
        var coachCountBotbash = 1;
        var daysBotbash = [[2, 4, 6]];
        var timesBotbash = [["8:45pm", "8:45pm", "8:45pm"]];
        createSchedules(dbHandle, channels, cM.scheduleChannelBotbash, coachCountBotbash, daysBotbash, timesBotbash, monAndSun);
    },
    /**
    ...
    updateSchedulePost: async function (schedule, channel) {
        ...
        for (let i = 0; i < old_embed.fields.length; i++) {
            ...
            for (let j = 0; j < lines.length; j++) {
                var line = lines[j];
                if (line.indexOf(schedule.emoji) === -1 ||
