 ...
 */
function createInhouseTeams(playerPositionMap, openUsers) {
    // now sort by tier
    ...
    while (true) {
        ...
        openUsers = openUsers.filter((usr) => {
            if (usr.id == playerPositionMap[pos][0].id ||
...
function createNonCompetitionTeams(playerPositionMap, openUsers) {
    var playersPerPosition = getPlayersPerPosition(openUsers);
    while (true) {
        // we're finished cause there are no more positions to fill
        if (playersPerPosition.length == 0)
...
module.exports = {
    ...
    createTeams: function (users, lobbyType) {
        var playerPositionMap = [[], [], [], [], []]; // all positions empty; for 1 team every pos gets one player, for two teams two players
        var openUsers = users;
