...
function validateTime(timeString) {
    var res = { hour: undefined, minute: undefined };
    // check length
    var l = timeString.length;
    if (l != 6 && l != 7)
        return res;
    // check sanity
    var hour = -1, minute = 0, ampm = "";
    if (l == 6) {
...
function getTimeString(zonedTime) {
    return (weekDays[zonedTime.dayOfWeek] +
        ", " +
...
module.exports = {
    ...
    getNextMondayAndSundayDate: function () {
        var now = new Date();
        var day = now.getDay(), diffToMondayNextWeek = day == 0 ? 1 : 8 - day, diffToSundayOfNextWeek = diffToMondayNextWeek + 6;
        return {
    ...
    getScheduledDate: function (mondayDate, day, timeString, timezoneName) {
        const hourAndMinute = validateTime(timeString);
        ...
        scheduledDate.setMinutes(hourAndMinute.minute - scheduledDate.getTimezoneOffset()); // remove time zone offset of server, offset given in minutes
    ...
    createLobbyTime: function (time, timezoneName) {
        ...
        // get time
        const hourAndMinute = validateTime(time);
        if (hourAndMinute.hour === undefined ||
        ...
        var timeDif = (hourAndMinute.hour - zonedDate.hours) * 1000 * 60 * 60 +
