...
function getTimeZoneShortNames(timezoneNames) {
  ...
  timezoneNames.forEach((name) => {
    ...
    if (abbr === "+08") abbr = "SGT";
    else if (abbr[0] == "+" || abbr[1] == "-") abbr = "UTC" + abbr;
...
function validateTime(timeString) {
  ...
  if (l != 6 && l != 7) return [undefined, undefined];
  ...
  if (ampm === "am") {
    ...
    // 12:30 am => 00:30
    else return [hour, minute]; // 1am = 1:00, 10am = 10:00
  } else {
    if (hour === 12) return [hour, minute];
    // 12:30pm = 12:30
    else return [hour + 12, minute]; // 1:00pm = 13:00, 11pm = 23:00
  }
}
...
module.exports = {
  ...
  getNextMondayAndSundayDate: function () {
    ...
    return [
      new Date(Date.now() + diffToMondayNextWeek * msPerDay),
      new Date(Date.now() + diffToSundayOfNextWeek * msPerDay),
    ];
  },
  ...
  createLobbyTime: function (time, timezoneName) {
    ...
    // get time zone
    [zone, error] = findTimeZone(timezoneName);
    if (zone === undefined) return [false, undefined, timezoneName, error];
    ...
    var timeDif =
      (hour - zonedDate.hours) * 1000 * 60 * 60 +
      (minute - zonedDate.minutes) * 1000 * 60;
    if (timeDif < 0)
  ...
  getZonedTimeFromTimeZoneName: function (date, timezoneName) {
    ...
    if (zone === undefined) return error;
